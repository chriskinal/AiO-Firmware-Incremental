...
SerialManager::SerialManager(HardwareManager &hardwareManager, DiagnosticManager &diagnosticManager)
    : hardwareManager_(hardwareManager), diagnosticManager_(diagnosticManager), gps1_(nullptr), gps2_(nullptr), rtk_(nullptr), esp32_(nullptr), rs232_(nullptr), imu_(nullptr), usb1DTR_(false), usb2DTR_(false), prevUSB1DTR_(false), prevUSB2DTR_(false), gps1Baud_(HardwareManager::BAUD_GPS), gps2Baud_(HardwareManager::BAUD_GPS), bufferOverflowDetected_(false), gps1Usage_("GPS1"), gps2Usage_("GPS2"), rtkUsage_("RTK"), esp32Usage_("ESP32"), rs232Usage_("RS232")
{
}
...
bool SerialManager::isBufferNearFull(HardwareSerial *port) const
{
    ...
    // Estimate buffer usage - this is a rough approximation
    // Real implementation would need access to internal buffer sizes
    return port->available() > 100; // Threshold for "near full"
}
readGPS1()
{
    gps1Usage_.timeIn();
